...
- Day/Month order (this can only be checked if a transition occurs in the input file, e.g. from 06-30 to 07-01)
...
def _check_dates_dt(ds: pd.Series)->int:
    '''
...
def _try_day_month_swap(ds: pd.Series)->pd.Series:
    '''
...
def _convert_to_dt_robust(ds: pd.Series)->pd.Series:
    '''
...
def _check_datetime_cols(df: pd.DataFrame)->pd.DataFrame:
    '''
...
def _cull_on_column(ds: pd.Series, ncull=100)->np.typing.NDArray[bool]:
    '''
    Return a bool array to be used for selection, based on column ds.
...
def _cull_data(df: pd.DataFrame, ncull=100)->pd.DataFrame:
    '''
...
def _read_file(filename: str, utf_codec: str, nskiprows: int, nrows=None, header=None)->pd.DataFrame:
    '''
    ...
    df = pd.read_csv(filename, header=header, skiprows=nskiprows, nrows=nrows, 
                encoding=utf_codec, infer_datetime_format=True,
                engine='python', parse_dates=[0], skipinitialspace=True,
                **xtraargs)
    return df

def _find_codec(filename: str, npeekrows=20)->str:
    '''
...
def _find_number_preamble_lines(filename: str, utf_codec: str, nskiprows=20)->tuple[int,int]:
    '''
...
def cull_gauge_file(filename:str, ncull=100)->pd.DataFrame:
    '''
